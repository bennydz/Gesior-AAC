	...
	$('#doubleStatus').live("click", function() {
		$.ajax({
			...
			success: function(data) {
				if (data.status == "success") {
					if (data.points == "on") {
						$('#doubleStatus').html('<img src="layouts/monteiro/images/shop/on.png" width="47px" height="23px" title="Ativo">');
						$('#doublePointsSelector').html('<div class="ribbon-double"></div>');
					} else {
						$('#doubleStatus').html('<img src="layouts/monteiro/images/shop/off.png" width="47px" height="23px" title="Desativado">');

...
a) The ' . htmlspecialchars($config['server']['serverName']) . ' is an alternative Tibia server planned, built and implemented within the existing rules that allow the operation thereof without breaking any law (whether national or not).
...
c) The ' . htmlspecialchars($config['server']['serverName']) . ' struggle for stability, however, we can not prevent mistakes will happen.
d) The history of donations is being saved, meaning you will never be forgotten.
e) The name assigned to the central points exchanges - ' . htmlspecialchars($config['server']['serverName']) . ' Shop - is fictitious; the ' . htmlspecialchars($config['server']['serverName']) . ' not sell any kind of product.
...
c) In case of falls and (or) problems that cause the ' . htmlspecialchars($config['server']['serverName']) . ' go offline or waiting for three consecutive days or more , players who have chosen to charge your VIP time points will be re-credited in the same way, the lost days (counting from the third day).
...
f) The ' . htmlspecialchars($config['server']['serverName']) . ' offers and invests in basic security for the server , up to you to keep it safe; ie not responsible for their pertencens, characters and accounts.
